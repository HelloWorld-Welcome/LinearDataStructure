...
{
    EXIT, INSERT_FIRST, INSERT_LAST, PRINT, DELETE_FIRST, DELETE_LAST, DELETE_ALL, SEARCH_BY_VALUE
} EMY_MENU;
...
// 리스트 마지막에 노드 추가 
void InsertLast(struct ListNode** ppHead, const int value);
// 리스트의 특정 인덱스에 노드 추가 
void InsertByIndex(struct ListNode** ppHead, const int idx, const int value);
// 리스트 첫번째 요소 제거 
void DeleteFirst(struct ListNode** ppHead);
// 리스트 마지목 요소 제거 
void DeleteLast(struct ListNode** ppHead);
// 리스트 검색 
void SearchByValue(struct ListNode** ppHead, const int value);
// 리스트의 인덱스를 다시 재조정 하는 함수 
...
void EventLoopRun(void)
{
    ...
    while ((menu = PrintMenu()) != 0)
    {
        ...
        case SEARCH_BY_VALUE:
            printf("찾을 값 입력: ");
            scanf_s("%d%*c", &input_value);
            SearchByValue(&pHead, input_value);
            PressAnyKeyMsg();
            break;
...
void InsertByIndex(struct ListNode** ppHead, const int idx, const int value)
{
    ...
    if (*ppHead == NULL && idx != 0 || idx < 0)
    {
        printf("[ErrorLog]: 해당 인덱스에 데이터 노드를 추가할 수 없습니다.(Empty List 또는 idx가 음수)\n"); 
        return;
    }
    ...
    if (pIterator->pNext == NULL && pIterator->idx + 1 == idx)
    {
        InsertLast(ppHead, value);
        return;
    }

    printf("[ErrorLog]: 해당 인덱스에 데이터 노드를 추가할 수 없습니다.(인덱스 범위 초과)\n");
}
...
void SearchByValue(struct ListNode** ppHead, const int value)
{
    ...
    while (pIterator != NULL)
    {
        ...
        {
            printf("[DebugLog]: [Idx: %d] | [this: %p] | [Value: %d] | [pNext: %p]\n", 
                pIterator-> idx, pIterator, pIterator->data, pIterator->pNext);
            cnt++;
        }
        pIterator = pIterator->pNext;
    }
    ...
    if (cnt == 0)
        printf("리스트에 %d이 포함되어 있지 않습니다.", value);
}

void SortListIdx(ListNode** ppHead)
{
    ListNode* pIterator = (ListNode*)*ppHead;
    ...
    printf("[DebugLog]: ListNode IDX 정렬 완료!\n");
}
